...
const SMSSender = () => {
  ...
  //SMS Subject
  const [smsSubjects, setSmsSubjects] = useState(
    JSON.parse(localStorage.getItem("smsSubjects")) || []
  );
  ...
  const MAX_SMTP_CONFIGURATIONS = 20;
  ...
  const addNewSMTP = (event) => {
    ...
    if (smtpConfigurations?.length < MAX_SMTP_CONFIGURATIONS) {
      ...
      toast.error("Maximum limit of SMTP configurations reached (20)");
    }
  };
  ...
  const handleSaveSmsSubject = () => {
    if (smsSubject.trim() === "") {
      toast.error("Please enter a valid subject.");
      return;
    }
    setSmsSubjects((prevSmsSubjects) => [
      ...prevSmsSubjects,
      smsSubject,
    ]);
    localStorage.setItem(
      "smsSubjects",
      JSON.stringify([...smsSubjects, smsSubject])
    );
    setSmsSubject("");
  };

  const handleDeleteSmsSubject = (index) => {
    const updatedSmsSubjects = [...smsSubjects];
    updatedSmsSubjects.splice(index, 1);
    setSmsSubjects(updatedSmsSubjects);
    localStorage.setItem("smsSubjects", JSON.stringify(updatedSmsSubjects));
  };
  //////////
  ...
  const sendSMS = async (e) => {
    ...
    if (!smsMessages) {
      toast.error("You need to add an SMS(s)");
      return;
    }

    try {
      const toastId = toast.loading(
        `Sending SMS to ${numberCount === 1 ? `${numberCount.toLocaleString()} number` : `${numberCount.toLocaleString()} numbers`}`
      );
      ...
      // Section 3: SMS Subjects
      setSmsSubjects([]);
      localStorage.removeItem("smsSubjects");
      ...
      if (
        ...
        toast.error("Your SMTP is currently not sending SMS.", {
          duration: 90000,
        });
      }
              ...
              <div className="pl-3">
                ...
                <div className="font-normal text-gray-500">
                  <span className="text-sm">
                    Input one or more SMTPs (Max. 20).
                  </span>
                </div>
              </div>
            </div>
                ...
                <div className="relative w-full h-full mt-14">
                  ...
                  <div className="relative p-6 mt-4 bg-white border-t-2 border-blue-700 rounded-lg shadow sm:p-5">
                    <div className="p-6 bg-white border border-gray-300 shadow-lg rounded-xl">
                      ...
                      <span className="text-sm">
                        Add this: "{userPhone}" to your SMS if you want to
                        add each number in your lead to your SMS Message.
                      </span>
                    </div>
